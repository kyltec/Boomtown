...
module.exports = postgres => {
  return {
    ...
    async getTagsForItem(id) {
      const tagsQuery = {
        text:
          'SELECT id, name AS title FROM tags WHERE id IN (SELECT tagsid FROM itemtags WHERE itemid = $1)',
        values: [id]
      };
          ...
          try {
            ...
            client.query('BEGIN', err => {
              ...
              imageStream.on('end', async () => {
                ...
                const newItemQuery = {
                  text:
                    'INSERT INTO users(title, description, ownerid ) VALUES($1, $2, $3) RETURNING *',
                  values: [title, description, user.id]
                };

                const createdItem = await postgres.query(newItemQuery);
                ...
                const tagRelationshipQuery = {
                  text: `INSERT INTO itemtags(itemid, tagsid) VALUES(${tagsQueryString(
                    [...tags],
                    itemid,
                    ''
                  )}) `,
                ...
                const addTag = await postgres.query(tagRelationshipQuery);
